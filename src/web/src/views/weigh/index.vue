    ...
    <div>
        <el-row justify="center">
            <el-col :span="30">
                ...
                >
                    ...
                    <el-table-column prop="skey" label="设备号" width="120" />
                    <el-table-column prop="model" label="型号" width="120" />
                    <el-table-column label="量程" width="200">
                        <template #default="{ row }">
                            {{ `${row.minCapacity}-${row.maxCapacity}${ScaleUnitMap[row.unit]}` }}
                        </template>
                    </el-table-column>
                    <el-table-column label="分度值" width="120">
                        <template #default="{ row }">
                            {{ `${row.displayInterval}${ScaleUnitMap[row.unit]}` }}
                        </template>
                    </el-table-column>
        ...
        >
            ...
            >
                <template v-if="!isEdit">
                    <el-form-item label="设备号" prop="skey">
                        <el-input v-model="form.skey" placeholder="请输入设备号" />
                    </el-form-item>
        ...
        >
            <el-table :data="recordsData" border>
                ...
                <el-table-column prop="employeeId" label="操作员ID" width="120" />
        ...
        >
            ...
            >
                <el-form-item label="设备ID" prop="id">
                    <el-input-number v-model="searchForm.id" :min="1" placeholder="请输入设备ID" />
                </el-form-item>
            </el-form>
            <template #footer>
...
import { reqGetScales, reqAddScale, reqUpdateScale, reqGetRecords, reqGetScale } from "@/api/weigh";  // 添加 reqGetScale
...
// 表单验证规则
const rules = {
    skey: [{ required: true, message: '请输入设备号', trigger: 'blur' }],
...
const handleSearchSubmit = async () => {
    ...
    await searchFormRef.value.validate(async (valid: boolean) => {
        if (valid) {
            try {
                const scale = await reqGetScale(searchForm.id);
                if (scale) {
