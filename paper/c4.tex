...
称重数据处理作为软件中的核心功能，设计到多端数据处理、图像识别以及 MQTT 消息的配置等多个操作，这里对其进行更为详细的设计与分析。
...
称重数据在边端完成提交，称重数据在 MQTT Broker 处完成持久化，便视为称重完成。接着，远端监控 Broker，开始称重数据进一步处理，如远端称重数据处理流程活动图\ref{fig:远端称重数据处理流程活动图}所示。
...
5、前面步骤中没有发生异常，则持久化称重信息并发布处理成功的消息到 MQTT Broker；
